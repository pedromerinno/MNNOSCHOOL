...
export function SmartCombobox({
  ...
  React.useEffect(() => {
    if (isMultiple) {
      ...
      // For single mode, ensure query is cleared when value changes externally
      const currentSingleValue = !isMultiple ? (typeof valueProp === "string" ? valueProp : null) : null;
      if (typeof valueProp === 'string' && valueProp !== currentSingleValue) {
        setQuery("");
      } else if (valueProp === null && currentSingleValue !== null) {
        setQuery("");
      }
    }
  ...
  function toggleOption(opt: Option) {
    ...
    if (isMultiple) {
      ...
      const newValue = isSelected(opt.id) ? null : opt.id;
      commitChange(newValue);
      // Clear query when selecting in single mode to show selected option
      if (newValue) {
        setQuery("");
      }
      setOpen(false);
  ...
  function setOpen(next: boolean) {
    ...
    onOpenChange?.(next);
    if (next) {
      // Clear query when opening to show search field
      ...
      // Clear query when closing if single mode
      if (!isMultiple) {
        setQuery("");
      }
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {!isMultiple && singleValue && !open ? (
          // Show selected option label when closed in single mode
          <div className="flex-1 text-[hsl(var(--foreground))]">
            {baseOptions.find(o => o.id === singleValue)?.label || placeholder}
          </div>
        ) : (
          <input
            ref={inputRef}
            ...
            placeholder={!isMultiple && singleValue ? baseOptions.find(o => o.id === singleValue)?.label : placeholder}
            disabled={disabled}
            className={classNames(
      ...
      {open && (
        ...
        >
                    ...
                    const defaultRow = (
                      ...
                      >
                        ...
                        <div className="ml-2 flex items-center gap-2">
                          {opt.meta ? (
                            <span className={classNames("text-xs", active ? "opacity-80" : "text-[hsl(var(--muted-foreground))]")}>
                              {opt.meta}
                            </span>
                          ) : null}
                          {selected && (
          ...
          {footer ? (
            <div className="border-t border-[hsl(var(--border))] p-2 text-xs text-[hsl(var(--muted-foreground))]">
              {footer}
            </div>
          ) : null}
        </div>
