...
export const AccessManagement: React.FC<AccessManagementProps> = ({
  ...
  const fetchAccessItems = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.from('company_access').select('*').eq('company_id', company.id).order('tool_name');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (currentAccess) {
        ...
        const {
          error
        } = await supabase.from('company_access').update({
  ...
  const copyToClipboard = (text: string, message: string) => {
    navigator.clipboard.writeText(text).then(() => toast.success(message)).catch(() => toast.error('Falha ao copiar para a área de transferência'));
  };
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                {currentAccess ? 'Atualize as informações de acesso abaixo.' : 'Adicione as informações de uma nova ferramenta.'}
              </DialogDescription>
            </DialogHeader>
            <form onSubmit={handleSubmit}>
              <div className="grid gap-4 py-4">
                <div className="grid gap-2">
                  ...
                  <Input id="tool_name" name="tool_name" value={formData.tool_name} onChange={handleInputChange} required />
                </div>
                <div className="grid gap-2">
                  <Label htmlFor="url">URL</Label>
                  <Input id="url" name="url" value={formData.url} onChange={handleInputChange} placeholder="https://..." />
                </div>
                <div className="grid gap-2">
      ...
      {isLoading ? <div className="flex justify-center p-6">
          ...
          <CardContent className="p-0">
            <Table>
              ...
              <TableBody>
                {accessItems.map(item => <TableRow key={item.id}>
                    <TableCell>{item.tool_name}</TableCell>
                    <TableCell>
                      {item.url ? <a href={item.url.startsWith('http') ? item.url : `https://${item.url}`} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline flex items-center">
                          {item.url}
                        </a> : '-'}
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center space-x-2">
                        ...
                        <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => copyToClipboard(item.username, 'Usuário copiado!')}>
                    ...
                    <TableCell>
                      <div className="flex items-center space-x-2">
                        <span>••••••••</span>
                        <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => copyToClipboard(item.password, 'Senha copiada!')}>
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-copy"><rect width="14" height="14" x="8" y="8" rx="2" ry="2" /><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2" /></svg>
                        </Button>
                      </div>
                    </TableCell>
