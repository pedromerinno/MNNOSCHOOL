...
export const JobRolesManager: React.FC<JobRolesManagerProps> = ({
  ...
  if (jobRoles.length === 0 && !newRole) {
    return <Card>
        <CardContent className="p-6 text-center px-[40px] py-[40px]">
          <Briefcase className="h-12 w-8 mx-auto text-gray-400 mb-4" />
  ...
  return <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        {!newRole && !editingRole && <div className="flex gap-2">
            <Button onClick={handleAddRole}>
              <Plus className="h-4 w-4 mr-2" />
              Adicionar Cargo
            </Button>
            {jobRoles.length > 0 && <Button variant="outline" onClick={() => refreshJobRoles(true)} className="px-3">
      ...
      {jobRoles.length > 0 && !newRole && !editingRole && <Card>
          <JobRolesList jobRoles={jobRoles} onMoveRole={handleMoveRole} onShowDetails={role => {
        setSelectedRole(role);
        setShowDetailsDialog(true);
      }} onManageUsers={role => {
        setSelectedRole(role);
        setShowRoleUsersDialog(true);
      }} onEditRole={setEditingRole} onDeleteRole={handleDeleteRole} />
        </Card>}
