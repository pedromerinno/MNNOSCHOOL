...
export const NotificationsWidget = memo(() => {
  ...
  // Format relative time (like "2 days ago") based on date
  ...
  // Improved initial fetch logic with proper debouncing
  useEffect(() => {
    // Only fetch when company changes and implement proper debounce
    if (selectedCompany?.id && 
      ...
      if (fetchTimeoutRef.current === null) {
        ...
        }, 1000); // Delay fetch by 1 second to prevent duplicate calls
      }
    }

    // Cleanup timeout on unmount or before setting new timeout
    return () => {
      if (fetchTimeoutRef.current !== null) {
      ...
      <CardContent className="p-0 flex flex-col h-full">
        <div className="p-8 flex justify-between items-center py-[24px]">
          ...
          <div className="flex space-x-2">
            <Button 
              ...
              className="h-7 w-7 rounded-full px-[25px] py-[25px] text-zinc-950 bg-[#ad7878]/0"
            >
        ...
        <div className="my-[35px]">
          {refreshing || isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-40 gap-2">
              ...
              {isAdmin && (
                <Button 
                  ...
                  size="sm" 
                  onClick={() => setDialogOpen(true)}
                  className="px-[20px] py-[25px] bg-zinc-950 hover:bg-zinc-800 text-zinc-50 rounded-full my-[20px]"
                >
                  Criar aviso
                </Button>
              )}
            ...
            <div className="px-[50px] py-0 flex flex-col items-center text-center">
