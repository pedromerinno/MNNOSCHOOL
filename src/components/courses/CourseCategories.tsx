...
const bgColors = ['bg-blue-100', 'bg-green-100', 'bg-purple-100', 'bg-orange-100', 'bg-pink-100', 'bg-teal-100', 'bg-indigo-100', 'bg-cyan-100'];
...
export const CourseCategories: React.FC<CourseCategoriesProps> = ({
  ...
  return <div ref={containerRef} className="flex items-center gap-3 flex-nowrap overflow-x-auto py-[20px]">
      {/* Visible categories */}
      ...
      return <div key={category.id} className={`inline-flex items-center gap-3 px-6 py-3 cursor-pointer transition-all rounded-3xl shrink-0
              ${isActive ? 'bg-black text-white' : `${randomBg} ${textColorMap[randomBg]} hover:opacity-80`}`} onClick={() => onCategoryChange(category.id)}>
            <span className="font-medium">{category.name}</span>
          </div>;
    })}

      {/* "+" button, only show if there are more categories than we can display */}
      {categories.length > visibleCategories.length && <Popover open={isOpen} onOpenChange={setIsOpen}>
          <PopoverTrigger asChild>
            <Button variant="outline" className="rounded-full flex-shrink-0 w-10 h-10 p-0" aria-label="Mais categorias">
          ...
          <PopoverContent className="w-[400px] max-h-[400px] overflow-y-auto p-4">
            ...
            return <div key={category.id} className={`flex items-center gap-3 px-4 py-2 cursor-pointer transition-all rounded-xl
                      ${isActive ? 'bg-black text-white' : `${randomBg} ${textColorMap[randomBg]} hover:opacity-80`}`} onClick={() => {
