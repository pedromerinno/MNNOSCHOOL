  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {courses.map(course => (
        <div 
          key={course.id} 
          className="group relative aspect-[4/3] rounded-lg overflow-hidden cursor-pointer shadow-sm hover:shadow-md transition-all duration-300" 
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/70 to-transparent px-[20px] py-[20px]">
            <div className="flex gap-2 mb-2">
              ...
              {course.tags?.slice(0, 2).map((tag: string, index: number) => (
                <span key={index} className="px-4 py-1.5 text-xs rounded-xl bg-white/20 text-white border border-white/40">
                  {tag}
                </span>
              ))}
              {/* Show "+" indicator if there are more than 2 tags */}
              {course.tags && course.tags.length > 2 && (
                <span className="px-4 py-1.5 text-xs rounded-xl bg-white/20 text-white border border-white/40">
