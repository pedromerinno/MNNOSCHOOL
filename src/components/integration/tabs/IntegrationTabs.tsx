...
export const IntegrationTabs: React.FC<IntegrationTabsProps> = ({
  ...
  return <div className="w-full">
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid grid-cols-4 w-full rounded-2xl p-1.5 gap-2 bg-gray-100/0">
          ...
          <TabsTrigger value="suggested-courses" className="flex items-center gap-2 rounded-xl py-4 px-6 transition-colors" style={{
        ...
        <div className="mt-10 mb-16 space-y-8">
          <TabsContent value="culture" className="m-0">
            <CultureManual companyValues={company?.valores || ""} companyMission={company?.missao || ""} companyHistory={company?.historia || ""} companyColor={companyColor} companyLogo={company?.logo} companyName={company?.nome} videoUrl={company?.video_institucional} videoDescription={company?.descricao_video} />
          </TabsContent>
